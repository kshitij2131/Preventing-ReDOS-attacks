...
nfa comp(nfa a){
	...
	for (auto x: a.q){
		bool pres = false;
		for (auto y : a.f){
			if (x==y) {pres=true; break;}
		}
		if (!pres) a_.f.insert(x);
	}
	a_.lan = a.lan;
	a_.q = a.q;
	a_.q0 = a.q0;
...
nfa un(nfa a, nfa b){
	if (a.q.empty()) return b;
	if (b.q.empty()) return a;
	nfa ans;
	...
	for (int i=0; i < sizeA*sizeB; i++) ans.q.push_back(i);
    ...
    for(auto &x: m){
        ...
        for(int i=0;i<vec.size();i++){
            for(int j=i+1;j<vec.size();j++){
                ...
                nfa x1 = StateReduction(inter(a1,a2));
                nfa x2 = StateReduction(concat(ap, x1));
                nfa x3 = StateReduction(concat(x2, comp(as)));
                ans = un(ans,x3);
